...
export async function POST(request: Request) {
  try {
    ...
    let sessionStatementsResponse: Array<{
      mode: string;
      statements: TheOpinionResponse[];
    }> = [];
    ...
    if (!checkExistenceOfTargetDir(targetSessionDir)) {
      try {
        ...
        return NextResponse.json(
          { dirPath: "Error creating directory" },
          { status: 500 },
        );
      }
    }
    ...
    try {
      fs.writeFileSync(
        targetFilePath,
        JSON.stringify(sessionStatementsResponse),
      );
      return NextResponse.json({ dirPath: targetFilePath });
    } catch (err) {
      console.error(`Error creating file: ${targetFilePath}`, err);
      return NextResponse.json(
        { dirPath: "Error creating file" },
        { status: 500 },
      );
    }
  } catch (error) {
    ...
    return NextResponse.json(
      { dirPath: "Error processing request" },
      { status: 500 },
    );
  }
}
